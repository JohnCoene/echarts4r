...
```{r}
e_matrix_raw(rows = 3, cols = 3)
```

All tables are fully customizable through the supported css options found [here](https://echarts.apache.org/en/option.html#matrix.body). We can use these options to fully remove the lines of the table leaving an blank matrix.

```{r}
e_matrix_raw(rows = 3, cols = 3, backgroundStyle=list(borderWidth=0), body = list(itemStyle = list(borderWidth = 0)))
```
