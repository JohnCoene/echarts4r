...
df %>% 
  ...
  e_datazoom(x.index = c(0, 1)) # add data zoom for for x axis
```
...
df %>% 
  ...
  e_angle_axis() %>% 
  e_radius_axis(x) %>% 
  e_line(y, coord.system = "polar") %>% 
  e_scatter(z, coord.system = "polar")
```

This will also work with different chart types.
